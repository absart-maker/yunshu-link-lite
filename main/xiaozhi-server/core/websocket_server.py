...
from core.engine_runtime import create_engine_channel, read_engine_mode
...
class WebSocketServer:
    def __init__(self, config: dict):
        self.config = config
        self.engine_mode = read_engine_mode(config)
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        # 引擎可切换模式：engine/auto 在可完整解析 Provider 时接管连接
        if self.engine_mode in ("engine", "auto"):
            headers = dict(websocket.request.headers)
            device_id = headers.get("device-id", "") or ""
            channel = create_engine_channel(
                websocket, self.config, device_id=device_id
            )
            if channel is not None:
                self.logger.bind(tag=TAG).info(
                    f"引擎模式 ({self.engine_mode}) 接管连接: {device_id}"
                )
                try:
                    await channel.run()
                finally:
